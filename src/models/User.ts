...
export interface UserModel extends Model<UserDocument> {
    updateOrCreate(query: UserQuery, updateFields: UserUpdateQuery): Promise<UserDocument>;
}
...
UserSchema.statics.updateOrCreate = async function (query: UserQuery, updateFields: UserUpdateQuery): Promise<UserDocument> {
    const user = await this.findOneAndUpdate(query, updateFields, { new: true, upsert: true })
    return user
}
